...
export default function Quiz() {
  ...
  if (id === 'new') {
    return (
      <div style={{ padding: '2rem' }}>
        ...
        <p>Aceasta este o pagină placeholder pentru crearea unui chestionar nou.</p>
        <p>Pentru demo, folosește butonul Înapoi pentru a reveni la listă.</p>
        <button onClick={() => navigate(-1)} style={{ marginTop: '1rem' }}>Înapoi</button>
      </div>
    );
  }
  ...
  function simulateProgress() {
    const existing = JSON.parse(localStorage.getItem(`progress_${quiz.id}`) || 'null');
    const currentCompleted = existing?.completed ?? 0;
    const currentCorrect = existing?.correct ?? 0;
    const currentWrong = existing?.wrong ?? 0;
    const sample = { completed: Math.min(quiz.totalQuestions, currentCompleted + 3), correct: currentCorrect + 2, wrong: currentWrong + 1 };
    localStorage.setItem(`progress_${quiz.id}`, JSON.stringify(sample));
    alert('Progres salvat (simulat). Revino pe pagina principală pentru a vedea sigla actualizată.');
  }
